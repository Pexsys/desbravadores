...
class CHAMADA extends TCPDF {
	...
	function __construct() {
		...
		$this->SetKeywords('Chamada, ' . str_replace(" ", ", ", $GLOBALS['pattern']->getClubeDS( array("db","nm","ig") ) ));
	...
	private function addLine($f){
		...
		if ( strftime("%m",strtotime($f["DT_NASC"])) == $this->mesAtu ):
			...
			$this->Cell($x-39, $h, fStrZero($f["ID_MEMBRO"],zeroSizeID())."-".$f["NM"], 0, false, 'L', true, false, 1);
			...
			$this->Cell($x-27, $h, fStrZero($f["ID_MEMBRO"],zeroSizeID())."-".$f["NM"], 0, false, 'L', true, false, 1);
