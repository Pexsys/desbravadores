...
class LISTAEVENTOBUS extends TCPDF {
	...
	public function Footer() {
 	...
 	public function Header() {
		...
		$this->Cell(185, 6, "LISTAGEM DE PASSAGEIROS - ÔNIBUS ". $this->header["CD"], 0, false, 'C', false, false, false, false, 'T', 'M');
		$this->posY += 8;
...
$result = $GLOBALS['conn']->Execute("
	SELECT es.ID, es.DS, es.DS_TEMA, es.DS_ORG, es.DS_DEST, esb.CD
      FROM EVE_SAIDA es
INNER JOIN EVE_SAIDA_BUS esb ON (esb.ID_EVE_SAIDA = es.ID)
     WHERE es.ID = ?
    ORDER BY esb.CD
", array($eveID) );
