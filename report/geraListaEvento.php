...
class LISTAEVENTOALFA extends TCPDF {
	...
	public function addLine($f){
		...
		$this->setXY(5, $this->posY);
		$this->Cell(85, 5, ($f["TP_REGIME"] == 'S' ? "* " : "") . $f["NM"], 0, false, 'L', true, false, 1);
		$this->setX(90);
	...
	public function addLineCount($result,$eveID){
		...
		$result = CONN::get()->Execute("
		    ...
		    WHERE ca.TP_REGIME = 'S'
			  AND es.ID = ?
		", array($eveID) );
		if (!$result->EOF):
			$this->setXY(150, $this->posY);
			$this->Cell(55, 6, "* Vegetarianos: ".$result->fields["QTD"], 0, false, 'R', true);
		endif;
	}
...
$result = CONN::get()->Execute("
	SELECT
			...
			ca.NM, at.CD_CARGO, IF(ca.TP_SEXO='F',cg.DSF,cg.DSM) AS DS_CARGO, ca.DT_NASC, ca.FONE_RES, ca.FONE_CEL, ca.IDADE_HOJE, ca.TP_REGIME,
