...
class LISTAINVESTIDURADSA extends TCPDF {
	...
	public function Footer() {
 	...
 	public function Header() {
		...
		$this->Cell(185, 9, "Listagem de Pessoas por Item - DSA", 0, false, 'C', false, false, false, false, 'T', 'M');
		$this->setXY(20,15);
...
$result = $GLOBALS['conn']->Execute("
    SELECT DISTINCT 1 AS ORD, ta.TP_ITEM, ta.CD_ITEM_INTERNO, ta.DS_ITEM AS X, cc.NM, ta.DS_ITEM
    FROM CON_COMPRAS cc
    INNER JOIN TAB_APRENDIZADO ta ON (ta.ID = cc.ID_TAB_APREND)
    WHERE ta.ID_DSA IS NOT NULL
      AND ta.TP_ITEM = 'CL'
    UNION ALL
    
    SELECT DISTINCT 2 AS ORD, ta.TP_ITEM, ta.DS_ITEM AS X, ta.CD_ITEM_INTERNO, cc.NM, ta.DS_ITEM
    FROM CON_COMPRAS cc
    INNER JOIN TAB_APRENDIZADO ta ON (ta.ID = cc.ID_TAB_APREND)
    WHERE ta.ID_DSA IS NOT NULL
      AND ta.TP_ITEM = 'ES'
    ...
    SELECT DISTINCT 3 AS ORD, ta.TP_ITEM, ta.DS_ITEM AS X, ta.CD_ITEM_INTERNO, cc.NM, ta.DS_ITEM
    FROM CON_COMPRAS cc
    INNER JOIN TAB_APRENDIZADO ta ON (ta.ID = cc.ID_TAB_APREND)
    WHERE ta.ID_DSA IS NOT NULL
      AND ta.TP_ITEM = 'ES'
