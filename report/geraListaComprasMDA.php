...
class LISTAINVESTIDURAMDA extends TCPDF {
            ...
            $this->add("
                <table cellpadding=\"3\" border=\"0\" cellspacing=\"0\">
                    ...
                    <tr>
                        ...
                        <td width=\"7%\" style=\"border-left:1px solid black;border-bottom:1px solid black;text-align:center;font-weight:bold;color:#000000;background-color:#C2C2C2\">Pág.</td>
                        <td width=\"86%\" style=\"border-left:1px solid black;border-bottom:1px solid black;border-right:1px solid black;color:#000000;font-weight:bold;background-color:#C2C2C2\">Nome</td>
                    </tr>
            ");
	...
	public function download() {
		$this->lastPage();
		$this->Output("ListagemInvestiduraMDA_".date('Y-m-d_H:i:s').".pdf", "I");
	}
}
...
$result = $GLOBALS['conn']->Execute("
	SELECT DISTINCT
		...
		ta.DS_ITEM
	FROM CON_ATIVOS at
	INNER JOIN APR_HISTORICO ah ON (ah.ID_CAD_PESSOA = at.ID AND ah.DT_AVALIACAO IS NOT NULL AND ah.DT_INVESTIDURA IS NULL)
	INNER JOIN TAB_APRENDIZADO ta ON (ta.ID = ah.ID_TAB_APREND)
	INNER JOIN TAB_MATERIAIS tm ON (tm.ID_TAB_APREND = ta.ID)
	WHERE NOT EXISTS (
				SELECT 1
				FROM APR_HISTORICO a
			INNER JOIN TAB_APRENDIZADO t ON (t.ID = a.ID_TAB_APREND)
			INNER JOIN TAB_MATERIAIS m ON (m.ID_TAB_APREND = t.ID AND m.TP IN ('DISTINTIVO','INSIGNIA') )
			LEFT JOIN CAD_COMPRAS_PESSOA c ON (c.ID_TAB_MATERIAIS = m.ID AND c.ID_CAD_PESSOA = a.ID_CAD_PESSOA)
			WHERE t.CD_ITEM_INTERNO = ta.CD_ITEM_INTERNO
			AND a.ID_CAD_PESSOA = at.ID
			AND a.DT_AVALIACAO IS NOT NULL 
			AND a.DT_INVESTIDURA IS NULL
			AND c.FG_COMPRA = 'S')
	ORDER BY 1, 2, 3, 4
");
