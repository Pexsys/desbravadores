...
function getDomainFilter( $parameters ) {
	...
	elseif ( $type == "SA" ):
		$domain = array(
			array( "id" => "P", "ds" => "PENDENTE", "icon" => "far fa-star-half" ),
			array( "id" => "C", "ds" => "CONCLUÍDO", "icon" => "far fa-star" ),
			array( "id" => "L", "ds" => "LIBERADO", "icon" => "fas fa-star" )
		);
...
function addFilter( $parameters ){
	...
	$str .= (!empty($icon) ? "<i class=\"pull-left $icon\"></i> " : "" ) ."&nbsp;<label for=\"optFilter$value\">$label:&nbsp;</label>";
