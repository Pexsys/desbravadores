...
class HISTORICO {
	...
	public static function dateDefaultInicio($date = null){
		return ( is_null($date) ? date( "Y-m-d", mktime( 0, 0, 0, 0, 1, date("Y") ) ) : $date );
	}

	public static function getParamDates( $frm ) {
		return array(
			"dt_inicio"			=> $frm["fg_inicio_alt"] == "S"			? HISTORICO::dateDefaultInicio(getDateNull($frm["dt_inicio"])) : "N",
	...
	public static function marcaRequisitoID( $assDT, $histID, $reqID ) {
	...
	public static function updateHistoricoQuery( $paramDates, $sWhere ){
	...
	public static function update( $barpessoaid, $barfnid, $paramDates) {
		$str = "";
		$id = null;
		$arrayDB = array();
		...
		if ($rh->EOF):
			$arrayDB = array( $barpessoaid, $barfnid, ($paramDates["dt_inicio"] == "N" ? HISTORICO::dateDefaultInicio() : $paramDates["dt_inicio"]) );
		...
		//SE EXISTE, ATUALIZA
			...
			if ($paramDates["dt_inicio"] != "N"):
				...
				$arrayDB[] = is_null($paramDates["dt_inicio"]) ? HISTORICO::dateDefaultInicio() : $paramDates["dt_inicio"];
				$bUp = true;
			endif;
			if ($paramDates["dt_conclusao"] != "N"):
		...
		if ( $tpItem == "ES" && $cdInte != "ME" && $paramDates["dt_conclusao"] != "N" ):
			HISTORICO::regraRequisitoEspecialidade($barfnid, $barpessoaid, $paramDates["dt_inicio"], $paramDates["dt_conclusao"] );
		endif;
		...
		if ( $tpItem == "CL" ):
			...
			", array($barpessoaid, $barfnid) );
			foreach($rs as $ks => $fS):
				HISTORICO::regraRequisitoEspecialidade($fS["ID_RQ"], $barpessoaid, $fS["DT_INICIO"], $fS["DT_CONCLUSAO"] );
			endforeach;
	    endif;

		$consulta = HISTORICO::consultaAprendizadoPessoa( $barfnid, $barpessoaid );
		return array(
			"op" => $str,
			"id" => $id,
			"ar" => $consulta["ar"],
		...
		foreach ($rg as $lg => $fg):
			...
			*/
			...
			if ( $fg["MIN_AREA"] > $feitas ):
				HISTORICO::marcaRequisitoID( null, $rI->fields["ID"], $fg["ID_TAB_APR_ITEM"] );
			...
			//SE ATINGIU O MINIMO PARA A AREA
				...
				if ($fg["TP_ITEM"] == "CL"):
					if ($dtConclusao >= $rI->fields["DT_INICIO"]):
						HISTORICO::marcaRequisitoID( $dtConclusao, $rI->fields["ID"], $fg["ID_TAB_APR_ITEM"] );
					endif;
				...
				elseif ($fg["TP_ITEM"] == "ES"):
					...
					if ($rI->EOF || is_null($rI->fields["DT_CONCLUSAO"]) ):
						...
						HISTORICO::update( $barpessoaid, $fg["ID"],
							array(
								"dt_inicio"			=> HISTORICO::dateDefaultInicio(getDateNull($dtInicio)),
								"dt_conclusao"		=> "N",
								"dt_avaliacao"		=> "N",
								"dt_investidura"	=> "N"
								) );
	...
	public static function analisePessoa($pessoaID){
		$arr = array();
		$alterados = 0;
		$deletados = 0;
