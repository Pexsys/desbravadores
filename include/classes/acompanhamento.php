...
class ACOMPANHAMENTO {
	...
	public static function marcaRequisito( $pessoaID, $itemID, $reqCD, $assDT ) {
		...
		if (!$rs->EOF):
			...
			HISTORICO::marcaRequisitoID( $assDT, $uh["id"], $arr["idreq"] );
		endif;

		HISTORICO::analisePessoa($pessoaID);
	...
	public static function get( $param ) {
		...
		$cap = HISTORICO::consultaAprendizadoPessoa( $param["it"], $param["ip"] );
		$str .= ACOMPANHAMENTO::getClass( $cap, $param );
	...
	public static function getClass( $cap, $param ){
		$strC = "";
		...
		$result = CONN::get()->Execute("
			...
			LEFT JOIN APR_PESSOA_REQ apr ON (apr.ID_HISTORICO = ah.ID AND apr.ID_TAB_APR_ITEM = tai.ID)
		     ...
		     ORDER BY tai.CD_REQ_INTERNO
		", array( $param["ip"], $param["it"] ) );
		$strC .= ACOMPANHAMENTO::getDetail("panel-success","Itens Conclu&iacute;dos","fa-smile-o",$result);
	...
	public static function getDetail($class, $titulo, $icon, $result){
		$str = "";
		if (!$result->EOF):
