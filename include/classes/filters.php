...
class FILTERS {
	...
	public static function Data( $parameters ){
		$temFiltro = false;
		...
		if (isset($flt) && !empty($flt)):
			...
			foreach ($aParam as $k):
				...
				if ($key !== false):
					$temFiltro = true;
					$arrFilter = FILTERS::add( array( "type" => $pFilters[$key]["id"], "desc" => $pFilters[$key]["ds"], "opsl" => $aOpSel ) );
	...
	public static function getDomain( $parameters ) {
		$domain = array();
		...
		//MES ANIVERSARIO
		elseif ( $type == "MA" ):
			$domain = DOMAINS::getMesAniversario();
		...
		//TIPO DE MATERIAIS
		elseif ( $type == "HT" ):
			$domain = DOMAINS::getTipoMateriais();
		...
		//UNIDADE
		elseif ( $type == "EV" ):
			$domain = DOMAINS::getEventos();

		//UNIDADE
		elseif ( $type == "U" ):
			$domain = DOMAINS::getUnidades();

		//TIPO APRENDIZADO
		elseif ( $type == "Z" ):
			$domain = DOMAINS::getTipoAprendizado();

		//CLASSE
		elseif ( $type == "C" ):
			$domain = DOMAINS::getClasses();

		//MESTRADOS
		elseif ( $type == "M" ):
			$domain = DOMAINS::getMestrados();

		//ESPECIALIDADES
		elseif ( $type == "E" ):
			$domain = DOMAINS::getEspecialidades();

		//MEMBROS ATIVOS
		elseif ( $type == "T" ):
			$domain = DOMAINS::getMembrosAtivos();

		//MEMBROS INATIVOS
		elseif ( $type == "I" ):
			$domain = DOMAINS::getMembrosInativos();

		//AREAS
		elseif ( $type == "A" ):
			$domain = DOMAINS::getAreasEspecialidades();
	...
	public static function add( $parameters ){
		$filter = FILTERS::getDomain( $parameters );
