...
class COMPRAS {
	...
	public static function deleteItemPessoaEntregue( $pessoaID, $itemAprendID ) {
		CONN::get()->Execute("
	...
	public static function deleteByPessoa( $pessoaID ) {
		CONN::get()->Execute("
			DELETE FROM CAD_COMPRAS
	...
	public static function insertItemCompra( $cd, $cadMembroID, $tp, $compl = null, $previsao = "N" ) {
		$r = CONN::get()->Execute("
			SELECT ID
			  FROM TAB_MATERIAIS
			 WHERE CD = ?
	...
	public static function processaListaMembroID( $cadMembroID, $tp ) {
		...
		foreach ($r1 as $k1 => $l1):
			...
			if ( $l1["TP_ITEM"] == "CL" ):
				...
				if ( $l1["CD_AREA_INTERNO"] == "REGULAR" ):
					...
					COMPRAS::insertItemCompra( $materialCD, $cadMembroID, $tp, null, $previsao );
					...
					if ( !empty($materialCD) ):
						...
						for ($qtd=1;$qtd<=$qtItens;$qtd++):
							...
							COMPRAS::insertItemCompra( $materialCD, $cadMembroID, $tp, $compl, $previsao );
						endfor;
					endif;
			...
			elseif ( $l1["TP_ITEM"] == "ES" ):
				...
				COMPRAS::insertItemCompra( $cd, $cadMembroID, $tp, null, $previsao );
			endif;
		endforeach;
	}
}
