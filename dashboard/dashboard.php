	...
	if ($mostra):
		...
		$rs = $GLOBALS['conn']->Execute("
		  ...
		  INNER JOIN TAB_PERFIL_ITEM tpi ON (tpi.ID_TAB_PERFIL = cuf.ID_PERFIL AND tpi.ID_TAB_DASHBOARD = 5)
			 WHERE cu.ID_USUARIO = ?
		",array( $_SESSION['USER']['id_usuario'] ) );
		if (!$rs->EOF):
	...
	<?php
		fDashBoard( "panel-red", "fa fa-thumbs-down fa-4x", 
			"SELECT COUNT(*) AS qtd FROM (
				...
				UNION ALL 
				SELECT 'CPF' AS INCONSIST FROM CON_ATIVOS WHERE (NR_CPF IS NULL OR LENGTH(NR_CPF) = 0) AND CPF_RESP IS NULL 
				UNION ALL 
